...
x <- lapply(names(group_list), function(group_name) {
  ...
  scdrake::md_header("Spatial plots", 3, extra = "{.tabset}")
  dimred_plots <- dplyr::distinct(df_group, name, .keep_all = TRUE)$dimred_plots[[1]]
    for (i in seq_len(nrow(dimred_plots))) {
    par <- dimred_plots[i, ]
    
    # Print Markdown header
    scdrake::md_header(str_to_upper("SPATIAL"), 4)
    
    # Extract the ggplot object from the list-column
    plot_obj <- par$plot[[1]]
