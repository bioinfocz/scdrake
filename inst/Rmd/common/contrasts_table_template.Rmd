...
```{r}
other_params <- scdrake_list(params$other_params)
```
...
```{r, results = "asis"}
x <- lapply_rows(other_params$dimred_plots, FUN = function(par) {
  md_header(str_to_upper(par$plot_dimreds), 3)
  p <- par$plot
  highlight_points(p, column_name = "colour_by", levels = c(other_params$target, other_params$reference)) %>% print()
})
```
