```{r, results = "asis"}
scdrake::generate_dimred_plots_section(
...
`r scdrake::format_used_functions(c("scater::runUMAP()", "scater::runTSNE()"))`
...
```{r, results = "asis"}
if (!rlang::is_null(cell_annotation_diagnostic_plots)) {
  scdrake::generate_dimred_plots_section(
    dimred_names = dp_dimred_names,
  ...
  scdrake::format_used_functions(c("SingleR::SingleR()", "SingleR::pruneScores()", "SingleR::plotDeltaDistribution()"), do_cat = TRUE)
  cat("\n\n#\n\n***")
}
```
