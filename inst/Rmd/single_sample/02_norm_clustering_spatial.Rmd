...
if (cfg$MANUAL_ANNOTATION) {
 scdrake::md_header("Manual cell annotation", 1, extra = "{.tabset}")
  ...
  cat("For manual annotation we modified an implemented function from the Giotto package. The enrichment Z score is calculated by using method (PAGE) from Kim SY et al., BMC bioinformatics, 2005 as",sep = "\n") 

  cat("$Z = \frac{((Sm – mu)*m^\frac{1}{2})}{delta}$.",sep="\n")
 
 cat("For each gene in each spot/cell, mu is the fold change values versus the mean expression
 and delta is the standard deviation. Sm is the mean fold change value of a specific marker gene set
 and  m is the size of a given marker gene set.",sep="\n")
cat("\n")
   
   if (cfg$MAKE_CELL_PLOT) {
     
  scdrake::create_a_link(href = drake::readd(plot_annotation)$anot_plot_out_pdf_file[2],text = glue("**Cell annotation plot**"), 
                         href_rel_start = fs::path_dir(cfg$NORM_CLUSTERING_REPORT_HTML_FILE),do_cat = TRUE)  
     }
cat("\n\n")
scdrake::create_img_link(drake::readd(plot_annotation)$anot_plot_out_pdf_file[1],img_src = drake::readd(plot_annotation)$anot_plot_out_png_file[1],img_width = "500px",href_rel_start = fs::path_dir(cfg$NORM_CLUSTERING_REPORT_HTML_FILE),do_cat = TRUE)
