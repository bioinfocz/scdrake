...
artifact_filtering <- cfg$ARTIFACT
...
```{r, fig.height = 10, fig.width = 8}
#plot_spat_visuals2 for HE image
...
SpotSweeper is an R/Bioconductor software designed for spatially-aware quality control of spatial transcriptomics (10x Visium) data. It identifies both local outlier spots and regional artifacts by comparing each spot's QC metrics—such as library size, gene count, and mitochondrial percentage, to those of its neighboring spot.
- localVariance(): Calculates the variance of a QC metric (e.g., library size, gene count) within each spot’s local neighborhood, helping identify spots with anomalous variation compared to their neighbors. Here, we are plotting distribution of the local variance in mitochondrial percentage. Samples affected by artifact ocurance often exhibit a long-tailed or bimodal distribution towards lower variance, reflecting regions with abnormal mitochondrial signal variation. If such distribution is present, we advise to re-run the scdrake with ARTIFACT: True option, to filter the spots affected by artefacts. Beware: if there is no indication of artifact in spots, the program will label half of the spots as artifact and remove them.

```{r, fig.height = 8, fig.width = 6}
pl <- ggplot2::ggplot(data.frame(colData(drake::readd(sce_unfiltered, path = drake_cache_dir))), aes(x = local_mito_variance_k36)) + 
  ggplot2::geom_density(fill = "gray", alpha = 0.5) 
pl
```
...
Plots removed spots after dataset-sensitive filtering.

```{r, fig.height = 10, fig.width = 8}
pl <- ggspavis::plotObsQC(drake::readd(sce_unfiltered, path = drake_cache_dir), plot_type = "spot", annotate = "discard_qc")
pl
```
...
Plots removed spots after custom filtering.

```{r, fig.height = 10, fig.width = 8}
pl <- ggspavis::plotObsQC(drake::readd(sce_unfiltered, path = drake_cache_dir), plot_type = "spot", annotate = "discard_custom")
pl
```

### Artifact

Ploting of possible artifact occurence in spots. If there is NO artifact in data, the program will label half of the spots as artifact. It is normal expected behaviour, in that case do not remove the spots. Check distribution of mitochondrial variance for artifact detection before spot removal. We do not advise to automatically removed spots before quality checks.  

```{r, results = "asis"}
if (drake::readd("config_input_qc",path = drake_cache_dir)$ARTIFACT) {
  pl <- ggspavis::plotObsQC(drake::readd(sce_unfiltered, path = drake_cache_dir), plot_type = "spot", annotate = "artifact")
  pl
} else {
  cat("Artifact was not detected or removed.")
}
```
