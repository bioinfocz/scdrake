...
details_template <- str_line(
  "\n<details>",
  "  <summary class='config'>{title}</summary>",
  "  <hr />",
  "  {text}",
  "  <hr />",
  "</details>\n"
...
The `multiBatchNorm()` function recomputes log-normalized expression values after
...
if (!is_null(hvg_int_with_cc)) {
  ...
  catg0(
    ...
    # hvg_rm_cc_genes_ids = str_comma(hvg_rm_cc_genes_ids, space = TRUE)
    gene = str_comma(glue("{hvg_rm_cc_genes_ann$ENSEMBL} ({hvg_rm_cc_genes_ann$SYMBOL})"), space = TRUE)
  )
}

if (hvg_combination == "hvg_metric") {
  catg0(
    "**HVG metric (common for all samples)**: '{hvg_int$metric}' -> combined by {hvg_combination_fn}\n\n",
    hvg_combination_fn = dplyr::if_else(hvg_int$hvg_metric == "gene_var", "combineVar()", "combineCV2()")
  )
  ...
  if (hvg_selection == "top") {
    ...
    catg0("{hvg_metric} > {hvg_selection_value}\n\n")
  }
} else {
  catg0("HVGs were combined by {hvg_combination} of HVGs within individual samples.\n\n")
}

invisible(lapply_rows(hvg_plots_int_df, FUN = function(par) {
  if (par$hvg_rm_cc_genes) {
    md_header("HVGs with removed CC genes", 2)
  } else {
    md_header("All HVGs", 2)
  }
  
  catg0("**Found {length(par$hvg_int$hvg_ids)} HVGs.**\n\n")
  print(par$hvg_plot)
  return(list())
}))
```
...
invisible(dplyr::group_by(sce_int_pca_df_for_report, name) %>% dplyr::group_map(function(group, key) {
  ...
  int_method_description <- get_int_method_description(int_method_name)
  md_header(int_method_description$header, 2, extra = "{.tabset}")
  ...
  dplyr::group_by(group, hvg_rm_cc_genes) %>%
    dplyr::group_map(function(group, key) {
      ...
      if (hvg_rm_cc_genes) {
        md_header("Removed cell cycle genes from HVGs", 3)
      } else {
        md_header("With all HVGs", 3)
      }
...
invisible(dplyr::group_by(sce_int_dimred_plots_df, name) %>% dplyr::group_map(function(group, key) {
  ...
  int_method_description <- get_int_method_description(int_method_name)
  md_header(int_method_description$header, 2, extra = "{.tabset}")
  catg0(details_template, title = "Method details", text = int_method_description$description)
  ...
  dplyr::group_map(by_hvg_rm_cc_genes, function(group, key) {
    ...
    if (hvg_rm_cc_genes) {
      md_header("Removed cell cycle genes from HVGs", 3, extra = "{.tabset}")
    } else {
      md_header("With all HVGs", 3, extra = "{.tabset}")
    }
    
    by_dimred <- dplyr::group_by(group, dimred_name)
    dplyr::group_map(by_dimred, function(group, key) {
      dimred_name <- key$dimred_name
      md_header(str_to_upper(dimred_name), 4)
...
invisible(dplyr::group_by(int_diagnostics_df, name) %>% dplyr::group_map(function(group, key) {
  ...
  dplyr::group_by(group, hvg_rm_cc_genes) %>%
    dplyr::group_map(function(group, key) {
      ...
      md_header("Cells's assignment", 4)
