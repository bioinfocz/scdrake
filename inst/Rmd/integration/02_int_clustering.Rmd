---
title: "scdrake | 02 - Post-integration clustering"
...
dp_selected_markers_files_rel_start <- if (!is_null(dp_selected_markers)) fs::path_dir(cfg$INT_CLUSTERING_REPORT_HTML_FILE) else NULL
...
pheatmap::pheatmap(
  table(
    drake::readd(cluster_int_graph_walktrap, path = drake_cache_dir)[[1]],
    drake::readd(cluster_int_graph_louvain, path = drake_cache_dir)[[1]]
  )
)
```

**Walktrap algorithm: found `r drake::readd(cluster_int_graph_walktrap_n, path = drake_cache_dir)` clusters.**

```{r, results = "asis"}
scdrake::cells_per_cluster_table_collapsed_html(drake::readd(cluster_int_graph_walktrap_table, path = drake_cache_dir))
```

**Louvain algorithm: found `r drake::readd(cluster_int_graph_louvain_n, path = drake_cache_dir)` clusters.**

```{r, results = "asis"}
scdrake::cells_per_cluster_table_collapsed_html(drake::readd(cluster_int_graph_louvain_table, path = drake_cache_dir))
```
...
```{r, results = "asis"}
scdrake::cells_per_cluster_table_collapsed_html(drake::readd(cluster_int_kmeans_kbest_table, path = drake_cache_dir))
```
...
for (k in cfg$KMEANS_K) {
  cluster_col <- glue::glue("cluster_int_kmeans_kc_{k}")
  scdrake::md_header(glue::glue("k = {k}"), 4)
  scdrake::cells_per_cluster_table_collapsed_html(cluster_int_kmeans_kc_tables[[cluster_col]])
}
```
...
```{r, results = "asis"}
for (k in cfg$SC3_K) {
  cluster_col <- glue::glue("cluster_int_sc3_{k}")
  scdrake::md_header(glue::glue("k = {k}"), 3)
  scdrake::cells_per_cluster_table_collapsed_html(cluster_int_sc3_tables[[cluster_col]])
