...
- Reading in feature-barcode matrix either from:
  - `cellranger` output
  - `spaceranger` output
  - `SingleCellExperiment` (SCE) object as a Rds file or as a target from an existing `{drake}` cache (i.e. from a different
    `{scdrake}` project)
  - Delimited table
...
- Identification and labeling of tissue artifact spots
...
- `type: "spaceranger"`: an output from [spaceranger](https://www.10xgenomics.com/support/software/space-ranger/latest), 
  files of a raw feature-barcode matrix - `barcodes.tsv`, `features.tsv`, and `matrix.mtx` (can be also gzipped having a `.gz` extension), 
  and directory spatial containing outputs that capture the spatiality of the data. This option result in creating SpatialExperiment object (SPE)
...
- `path: "path/to/file/or/dir"`: a path to input file or directory (can be relative to project root directory).
  ...
  - For `type: "spaceranger"`, this is a path to directory containing the `barcodes.tsv`, `features.tsv`, and `matrix.mtx`
  files (can be also gzipped having a `.gz` extension) and spatial directory with spaceranger spatial output (`tissue_lowres_image.png`, `tissue_positions_list.csv`)
...
In the input_qc stage, the spatial extension generates spatial quality control reports and estimates the probability of tissue artifacts using  `SpotSweeper::findArtifacts()`.
...
If `True`, pipeline enables removal of detected artifact.

We recommend running the pipeline once without artifact removal first, if no tissue artifact is detected, the function will randomly label half of the spots and remove them. 
...
`sce_raw`: an untouched SCE/SPE object as loaded from:

- `DropletUtils::read10xCounts()` (`cellranger` input)
- `SpatialExperiment::read10xVisium()` (`spaceranger` input)
- `readr::read_delim()` (delimited table input)
- `SingleCellExperiment` object (Rds file or `{drake}` cache)

***

`sce_valid_cells`: a SCE/SPE object with empty droplets removed
...
`sce_qc_filter`, `sce_qc_filter_genes`: a SCE/SPE object with cells filtered by dataset-sensitive filtering.
The latter with filtered genes.

`sce_custom_filter`, `sce_custom_filter_genes`: a SCE/SPE object with cells filtered by custom filtering.
The latter with filtered genes.
