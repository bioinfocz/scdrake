...
A threshold for minimum number of UMI per cell, e.g. cells with UMI less than `MIN_UMI_CF` will be removed.
...
A threshold for maximum number of UMI per cell, e.g. cells with UMI greater than `MAX_UMI_CF` will be removed.
...
e.g. cells with detected features less than `MIN_FEATURES` will be removed.
...
e.g. cells with mitochondrial genes detected in more than (`MAX_MITO_RATIO` * 100)% of all genes will be removed.
...
The latter is currently not used for filtering.
...
A threshold for minimum number of UMI per cell, e.g. genes with UMI < `MIN_UMI` will be removed.
...
e.g. genes expressed in less than (`MIN_RATIO_CELLS` * 100)% of cells will be removed.
