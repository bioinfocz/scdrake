...
tmp_dir <- tempfile()
scdrake::init_project(tmp_dir, set_active_project = FALSE, set_wd = FALSE)
```
...
- `config/`: config files in YAML format. See `vignette("config"`) for more details.
...
Open this file and set value of `INPUT_10X_DIR` to `"example_data/pbmc1k"`.
...
its value (object) is saved to cache (directory `.drake` by default), and you can anytime load it to your R session.
This is done via two `{drake}`'s functions: `drake::loadd()` loads target's value to the global environment,
and `drake::readd()` returns target's value (so it can be assigned to variable).
...
- `config/single_sample/01_input_qc.yaml`: set `INPUT_10X_DIR` to `"example_data/pbmc3k"`
