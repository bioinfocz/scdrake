...
output:
  BiocStyle::html_document:
    ...
    toc_float: true
vignette: >
  %\VignetteIndexEntry{scdrake config}
...
Configs are stored in YAML files (in `config/` directory by default), which are parsed in R to lists and used to build
`{drake}` plans. YAML ([quick reference](https://docs.ansible.com/ansible/latest/reference_appendices/YAMLSyntax.html),
...
during project initialization, update, and pipeline run (e.g. in `run_single_sample()` or `_drake_single_sample.R`).
...
(by default if you run e.g. `run_single_sample()`).
...
and then `FOO_2` will be updated, `BAR_3` will be added, but `BAR_2` will still be present, although it is not defined
in the local config!

## Merging of nested named lists

To overcome the problem with `FOO_NAMED_LIST`, parameters with such structure are specified as named list wrapped
by unnamed list:
...
A special type of parameter can be used to evaluate a value as R code:
...
By default, in stage configs (e.g. `02_norm_clustering`) you can also use parameters (variables) from `pipeline.yaml`
...
The presence of `yq` binary is checked on `{scdrake}` load or attach.
`SCDRAKE_YQ_BINARY` environment variable is read - if not set, a value from `Sys.which("yq")` is used (this function
searches in `PATH` environment variable). Then `scdrake_yq_binary` option is set, and is used as default value
to config-updating functions (see `?update_config`).
