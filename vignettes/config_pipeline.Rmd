...
In case of problems with `qs` use `rds` which is slower, but the most compatible.
...
Before pipeline is run by `run_single_sample_r()`, `run_single_sample_r()`, `run_integration_r()`, or
`run_integration()`, the `RSTUDIO_PANDOC` is set: a path to directory with [pandoc](https://pandoc.org/)'s binary.
...
You can also define this variable in `.Renviron` file and set it to `null` in pipeline config.
