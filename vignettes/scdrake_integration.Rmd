...
To do so, open `config/single_sample/02_norm_clustering.yaml` and set `CLUSTER_SC3_ENABLED` to `False`.
...
- In `~/scdrake_projects/pbmc3k`:
  ...
  - Optionally to save time, open `config/single_sample/02_norm_clustering.yaml` and set `CLUSTER_SC3_ENABLED` to `False`.
