...
paths to config directories. See `?get_scdrake_default_options()` and `?get_scdrake_options()` for more details and
...
Anyway, to replicate what is done inside `run_single_sample()`, you just need this code:
...
- You can modify `cfg_pipeline` and `cfg`, as these are lists (but with strict rules, see `?scdrake_list`).
  ...
  ([config_process_common.R](R/config_process_common.R), [config_process_single_sample.R](R/config_process_single_sample.R),
  [config_process_integration.R](R/config_process_integration.R)). This may change in the future and these functions
  will be exposed to package users.
...
you have to add names of your own targets to be made.
