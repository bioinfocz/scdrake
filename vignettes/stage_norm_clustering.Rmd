...
This stage build upon the `01_input_qc` stage and performs perhaps the most important steps in scRNA-seq and sequence-based SRT data analysis:
normalization, highly and spatially variable gene selection, dimensionality reduction, clustering, deconvolution, cell type annotation, and
visualization.
...
- Selection of [spatialy variable genes](https://lmweber.org/OSTA/pages/ind-feature-selection-testing.html) (SVGs):
  - SVGs are combined with HVGs so that both spatial patterns and overall expression variability are leveraged in dimensionality reduction and clustering
...
- Manual cell type annotation based on signatures score of specified cell type markers
...
- [Spot deconvolution](https://lmweber.org/OSTA/pages/seq-deconvolution.html):
  - [CARD](https://github.com/YMa-lab/CARD)
  - [RCTD](https://github.com/ggrajeda/spacexr)
...
This option enables pseudo tissue visualization and other visualization options for marker-based annotation.
...
#### Spatialy variable genes (SVGs) selection 

```yaml
SVG: False
```
When the SVG option is enabled, spatially variable genes are automatically identified alongside HVGs.
This is done using the nnSVG::nnSVG() function with the "MoransI" selection method. Moran’s I is a statistic that measures spatial autocorrelation—it detects whether expression values for a gene are more similar in nearby spots than would be expected by chance.

The resulting union of unique SVGs and HVGs is carried forward for downstream analyses, ensuring that both spatial structure and overall variability are preserved in dimensionality reduction and clustering steps.

For methodological details, see: <https://www.biorxiv.org/content/10.1101/2021.08.27.457741v1>.
...
To skip the cell annotation step, define `CELL_ANNOTATION_SOURCES` as `null`.
...
Path to user-define file contacting markers for each expected cell type. 
Each column name is one cell type, in rows are gene markers. Example: 
...
***
```yaml
ANNOTATION_CLUSTERING: "cluster_graph_leiden_r0.2"
```
...
Only together with spatial option. Whether to show pseudo tissue cell plots with enrichment of annotation. 
...
```yaml
SPOT_DECONVOLUTION: False
```

**Type:** logical scalar

Wether spot deconvolution for SRT data is enabled. Spot deconvolution is reference based, user must define reference scRNA-seq object and column in colData that contain information about cell types for each cell. Currently two main methods are available. 

***
```yaml
DECONV_METHOD: "CARD" (`"CARD"` | `"RCTD"`)
```
**Type:** character scalar

Which deconvolution method use. 
***

```yaml
REFERENCE_SC_DECONV: Null
```
**Type:** character scalar

Path to user-define reference `SingleCellExperiment` object in rds format used for spot deconvolution, containing expected cell types. 

***
```yaml
REFERENCE_SC_LABEL: Null
```
**Type:** character scalar

Column in colData in defined reference `SingleCellExperiment` object, that contains cell types labels/cell characteristics.
...
#NORM_CLUSTERING_REPORT_RMD_FILE: "Rmd/single_sample/02_norm_clustering_spatial.Rmd"
...
#### `SingleCellExperiment` or `SpatialExperiment` objects
...
***
`plot_deconv_results`: a tibble holding plots of deconvolution results, barplots as sumarizes statistics and spatial pseudo-tissue plots
***
...
cell annotation labels from the `cell_annotation_labels` target, manual annotation labels from the `manual_annotation_labels` target, deconvolution labels from the `spot_deconvolution_labels` target,  and the new cell groupings defined in
