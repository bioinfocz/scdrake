...
There can be only a subset of barcodes in the additional data - the missing ones will have `NA` set in the resulting data.
Rownames are not mandatory.
...
- `description: "Graph-based clustering ..."`: a description of the new assignment. If it was not set,
  the name of the assignment (`cluster_graph_leiden_r0.4_annotated`) would be used instead.
...
An example CSV file for PBMC 1k dataset can be retrieved by
`system.file("extdata", "selected_markers.csv", package = "scdrake", mustWork = TRUE)` or see
[here](https://github.com/bioinfocz/scdrake/blob/main/inst/extdata/selected_markers.csv).
