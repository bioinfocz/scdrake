...
create_signature_matrix_fn <- function(markers_file){
  ...
  list_markers2 <- lapply(list_markers, function(z){ z[!is.na(z) & z != ""]})
  #print(list_markers2)
  signature_matrix = makeSignMatrixPAGE(sign_names = c(names(markers)),
                                        sign_list = list_markers2)
...
meta_heatmap_ploting <- function(sce,clus_cor_method="pearson",clus_cluster_method = "complete",
                                 ...
                                 y_text_size = 10,
                                 strip_text_size = 8,
                                 low = 'blue', mid = 'white', high = 'red',spatial=FALSE,make_cell_plot=FALSE,
                                 out_dir=NULL) {
  ...
  #cell_types = colnames(enrichmentDT)
...
##plotMetaDataCellsHeatmap
  ...
  main_factor = ifelse(length(clustering) == 1, clustering, first_meta_col)
...
# for clusters
  ...
  #colData(sce) <- cbind(colData(sce),cell_metadata)
...
# for genes
  ...
  if (spatial) {
    variable = length(unique(colData(sce)[[clustering]]))
    palete = c(scales::hue_pal()(par$n_clusters))
    p <- visualized_spots(sce,cell_color=clustering,color_as_factor = F,
                          point_shape =  "border",cell_color_code = palete)
    pl <- cowplot::plot_grid(pl,p,ncol = 2, nrow = 1)
