...
.create_single_sample_dirs <- function(cfg) {
  dirs <- c("INPUT_QC_BASE_OUT_DIR", "NORM_CLUSTERING_BASE_OUT_DIR", "NORM_CLUSTERING_SELECTED_MARKERS_OUT_DIR")
  .create_dirs(cfg[dirs])
  invisible(NULL)
}

.create_integration_dirs <- function(cfg) {
  dirs <- c("INTEGRATION_BASE_OUT_DIR", "INTEGRATION_SELECTED_MARKERS_OUT_DIR", "INT_CLUSTERING_BASE_OUT_DIR")
  .create_dirs(cfg[dirs])
  invisible(NULL)
}
