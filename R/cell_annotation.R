...
cell_annotation_params_fn <- function(cell_annotation_sources, biomart_dataset = NULL) {
  ...
  cell_annotation_sources <- lapply(cell_annotation_sources, function(cell_annotation_source) {
    ...
    if (reference_type == "celldex") {
      ...
      reference_se <- tryCatch(
        withr::with_package("ensembldb", reference_se(ensembl = TRUE, cell.ont = cell_ont)),
        error = function(e) {
          if (stringr::str_detect(e$message, "failed to load resource")) {
            cli::cli_abort(str_space(
              "Failed to load {.pkg celldex} reference {.field {reference}} for {.pkg SingleR}.",
              "It is possible that the cached files are corrupted. You can try to remove the cache directory returned from",
              "{.code AnnotationHub::hubCache(ExperimentHub::ExperimentHub()) and re-run the pipeline."
            ))
          } else {
...
#'   ([details](https://bioconductor.org/books/3.15/SingleRBook/annotation-diagnostics.html#based-on-the-scores-within-cells)).
#'   Generated by [SingleR::plotScoreHeatmap()]
...
#'   ([details](https://bioconductor.org/books/3.15/SingleRBook/annotation-diagnostics.html#based-on-marker-gene-expression)).
...
#'    ([details](https://bioconductor.org/books/3.15/SingleRBook/annotation-diagnostics.html#based-on-the-deltas-across-cells))
#'
...
#' @param do_heatmaps A logical scalar: if `TRUE`, generate score heatmaps.
...
cell_annotation_diagnostic_plots_fn <- function(cell_annotation,
  ...
  if (do_heatmaps) {
    res <- res %>%
      dplyr::mutate(score_heatmaps_out_file = fs::path(.data$out_dir, "score_heatmaps.pdf"),)
  } else {
    res$score_heatmaps_out_file <- NA
  }
