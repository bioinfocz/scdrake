...
```{r install_stable, echo = FALSE, results = "asis"}
version <- desc::desc(file = "DESCRIPTION")$get_version()
gh_repo <- glue::glue("bioinfocz/scdrake@v{version}")
out <- c(
  '\n',
  '```{r, eval = FALSE}',
  ...
  '}',
  '',
  glue('BiocManager::install("bioinfocz/scdrake@v{version}")'),
  '```',
  '\n'
)
```
...
`{clustermq}` R package (probably only usable on Linux), which is used by `{drake}`.
However, it is also possible to use the `{future}` package, which doesn't require any additional system dependencies,
but it's a bit slower.

More reading about this topic can be found at <https://books.ropensci.org/drake/hpc.html>.

### Alternative storage format

`{drake}` supports several types of storage formats for cached objects, but `{qs}` is recommended and used by
`{scdrake}` by default due to its performance. You can install `{qs}` with
